...
# -> 여러 형식의 셀렉터를 동시에 처리해야 할 경우 콤마로 구분하여 지정한다.
...
for item in link_list:
     ...
     if "href" in item.attrs:
          ...
          # URL에 뉴스 상세 페이지의 파일명인 "read.nhn"이 포함되어 있다면 해당 주소를 url_list에 추가한다
...
for i, url in enumerate(url_list):
     ...
     if not news_html: # 가져온 내용이 없다면?
          ...
          for item in news_html:
               ...
               crawler.remove(item, "span", {"class" : "end_photo.org"})
...
# 워드 클라우드 객체 만들기
wc = WordCloud(font_path="NanumGothic", max_font_size=200, width=1200, height=800, scale=2.0, background_color="#ffffff")
